...
export default function HeroSection() {
  ...
  const proofPoints = [
    { label: t("home.hero.proof1Label", "Live court inventory"), value: "500+" },
    { label: t("home.hero.proof2Label", "Active players"), value: "10K+" },
    { label: t("home.hero.proof3Label", "Weekly team sessions"), value: "3.2K" },
  ];
      ...
      <div className="relative mx-auto grid min-h-[calc(100svh-72px)] max-w-7xl items-end gap-12 px-4 py-14 sm:px-6 lg:grid-cols-[1.2fr_0.8fr] lg:px-8 lg:py-18">
        ...
        >
          <div className="flex flex-wrap items-center gap-3 text-white/78">
            <span className="pill-label border-white/15 bg-white/8 text-white/82">
              {t("home.hero.badge", "Premium athletic booking")}
            </span>
            <span className="flex items-center gap-2 text-sm font-medium">
              <span className="status-dot" />
              {t("home.hero.flow", "Book, recruit, and play from one flow")}
            </span>
          </div>

          <div className="flex flex-col gap-5">
            <p className="section-kicker text-white/70">{t("home.hero.kicker", "Courtly")}</p>
            <h1 className="max-w-4xl text-5xl font-semibold leading-[0.94] text-white sm:text-6xl lg:text-7xl">
              {t("home.hero.line1", "Book the court.")}
              <br />
              {t("home.hero.line2", "Build the squad.")}
              <br />
              {t("home.hero.line3", "Keep the session moving.")}
            </h1>
            <p className="max-w-2xl text-base leading-7 text-white/74 sm:text-lg">
              {t(
                "home.hero.copy",
                "Discover high-quality venues, manage your next match, and keep team logistics in one sharp, mobile-first workspace.",
              )}
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <Button asChild size="lg" className="h-12 rounded-full px-6">
              <Link href="/courts">
                {t("home.hero.exploreCourts", "Explore courts")}
            ...
            >
              <Link href="/teams">{t("home.hero.browseTeams", "Browse teams")}</Link>
            </Button>
          </div>
        ...
        >
          <div className="flex items-center justify-between border-b border-white/12 pb-4">
            <div>
              <p className="section-kicker text-white/56">{t("home.hero.sessionBoard", "Session board")}</p>
              <h2 className="mt-1 text-2xl font-semibold text-white">{t("home.hero.tonight", "Tonight at 7:30 PM")}</h2>
            </div>
            <span className="rounded-full border border-emerald-400/25 bg-emerald-400/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.16em] text-emerald-200">
              {t("home.hero.available", "Available")}
            </span>
          </div>
          ...
          <div className="grid gap-4 py-5">
            <div className="rounded-[1.5rem] border border-white/10 bg-black/18 p-4">
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="rounded-2xl border border-white/12 bg-white/8 px-3 py-2 text-end">
                  <p className="text-xs uppercase tracking-[0.18em] text-white/48">{t("home.hero.rate", "Rate")}</p>
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <div className="rounded-[1.5rem] border border-white/10 bg-black/18 p-4">
                <p className="text-xs uppercase tracking-[0.18em] text-white/48">{t("home.hero.roster", "Roster")}</p>
                <div className="mt-3 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-white/64">{t("home.hero.playersConfirmed", "Players confirmed")}</p>
                  </div>
                </div>
              </div>

              <div className="rounded-[1.5rem] border border-white/10 bg-black/18 p-4">
                <p className="text-xs uppercase tracking-[0.18em] text-white/48">{t("home.hero.schedule", "Schedule")}</p>
                <div className="mt-3 flex items-center gap-3">
                  <div className="rounded-2xl border border-white/10 bg-white/8 p-3 text-primary">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-white/64">{t("home.hero.duration", "Warm-up to final whistle")}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <p className="text-sm leading-6 text-white/62">
              {t(
                "home.hero.summary",
                "Courtly keeps search, booking, team status, and reminders on one board so the game does not stall between messages.",
              )}
            </p>
          </div>
        </motion.div>
